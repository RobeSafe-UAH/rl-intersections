...
class LaneWaypointPlanner(LaneGraphPlanner):
    ...
    def __init__(self, map_data, map_flag):
        LaneGraphPlanner.__init__(self, map_data, map_flag)
        """
        Self parameters
        """
        ...
        self.current_waypoint = None
        self.goal_xyz = None
        self.distance = 3 # @TODO: This could be given as a parameter instead of hardcoded
        self.previous_lane_id = None

        """
        ROS Subscribers
        """
        ### --- Commented for CarlaChallenge2021 --- ###
        ...
        self.changeleft_subscriber = rospy.Subscriber(
            "/t4ac/src/classic/t4ac_petrinets_ros/t4ac_petrinets_ros_node/change_left",
            Bool, self.change_left)
        self.changeright_subscriber = rospy.Subscriber(
            "/t4ac/src/classic/t4ac_petrinets_ros/t4ac_petrinets_ros_node/change_right",
    ...
    def odometry_callback(self, odometry):
        ...
        current_waypoint_aux = builder_classes.T4ac_Waypoint()
        current_waypoint_aux.transform.location.x = self.current_xyz[0]
        current_waypoint_aux.transform.location.y = self.current_xyz[1]
        current_waypoint_aux.transform.location.z = self.current_xyz[2]
        self.current_waypoint = current_waypoint_aux.get_closer_wp(self.roadNetwork_waypoints)
    ...
    def change_left(self, order):
        """
        Callback for /t4ac/planning/change_left_order
        ...
        road = map_utils.get_road(self.map_object.roads, self.current_waypoint.road_id)
        s = self.current_waypoint.s

        # Get left waypoint
        if self.current_waypoint.lane_id > 0:
            left_lane = map_utils.get_lane(road, self.current_waypoint.lane_id - 1)
            left_waypoint = map_utils.generate_waypoint_in_lane(road, left_lane, s)
    ...
    def change_right(self, order):
        """
        Callback for /t4ac/planning/change_right_order
        ...
        # Get right waypoint
        if self.current_waypoint.lane_id > 0:
            right_lane = map_utils.get_lane(road, self.current_waypoint.lane_id + 1)
            right_waypoint = map_utils.generate_waypoint_in_lane(road, right_lane, s)
    ...
    def calculate_waypoint_route(self, distance, init_xyz, goal_xyz, weight="length"):
        ...
        if len(lane_route) == 1:
            ...
            if lane_route[0][2] == "lanefollow":
                road = map_utils.get_road(roads, lane_route[0][0])
                lane = map_utils.get_lane(road, lane_route[0][1], 0)
                waypoint_route += map_utils.generate_previous_waypoints_until_lane_start(
            ...
            if lane_route[0][2] == "lanefollow":
                road = map_utils.get_road(roads, lane_route[0][0])
                lane = map_utils.get_lane(road, lane_route[0][1], 0)
                waypoint_route += map_utils.generate_next_waypoints_until_lane_end(
            ...
            for i in range (1, len(lane_route)-1):
                if lane_route[i][2] == "lanefollow":
                    ...
                    lane = map_utils.get_lane(road, lane_route[i][1], 0)
                    waypoint_route += map_utils.generate_waypoints_in_lane(
                        road, lane, distance)

            # Calculate waypoints until last waypoint of the route in last step
            if lane_route[-1][2] == "lanefollow":
                road = map_utils.get_road(roads, lane_route[-1][0])
                lane = map_utils.get_lane(road, lane_route[-1][1], 0)
                waypoint_route += map_utils.generate_previous_waypoints_until_lane_start(
                    road, lane, distance, goal_waypoint.s)
        ...
        for waypoint in waypoint_route:
            distance_aux = planning_utils.euclidean_distance(
    ...
    def calculate_waypoint_route_multiple(self, distance, input_data, mode, weight="length"):
        ...
        for i in range(0, len(input_xy_tuples)):
            waypoint_aux = builder_classes.T4ac_Waypoint()
            waypoint_aux.transform.location.x = input_xy_tuples[i][0]
            waypoint_aux.transform.location.y = input_xy_tuples[i][1]
            waypoint_aux.transform.location.z = input_xy_tuples[i][2]
            waypoint = waypoint_aux.get_closer_wp(waypoints)
        ...
        if lane_route[0][2] == "lanefollow":
            road = map_utils.get_road(roads, lane_route[0][0])
            lane = map_utils.get_lane(road, lane_route[0][1], 'unknown')
            waypoint_route += map_utils.generate_next_waypoints_until_lane_end(
                road, lane, distance, init_waypoint.s)
        ...
        for i in range (1, len(lane_route)-1):
            if lane_route[i][2] == "lanefollow":
                road = map_utils.get_road(roads, lane_route[i][0])
                lane = map_utils.get_lane(road, lane_route[i][1], 'unknown')
                waypoint_route += map_utils.generate_waypoints_in_lane(
                    road, lane, distance)
        ...
        if lane_route[-1][2] == "lanefollow":
            road = map_utils.get_road(roads, lane_route[-1][0])
            lane = map_utils.get_lane(road, lane_route[-1][1], 'unknown')
            waypoint_route += map_utils.generate_previous_waypoints_until_lane_start(
                road, lane, distance, goal_waypoint.s)
...
def main():
    ...
    try:
        ...
        laneWaypointPlanner = LaneWaypointPlanner(map_name, 0)
        rospy.spin()
