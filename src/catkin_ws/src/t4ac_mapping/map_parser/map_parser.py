...
class MapParser:
    ...
    def __init__(self, map_data, map_flag = True):
        """
        Args
            map_name: (str) Name of the map without extension, i.e. 'Town01'
            map_data: (str) It can be given in two ways:
                            0) Name of the map without extension, i.e. 'Town01'
                            1) All map data saved as a string
            map_flag: (bool) If True is case 1), if False is case 2)
        """
        if map_flag == False:
            self.xodr_map = self._get_map(map_data)
        elif map_flag == True:
        ...
        self.map_name = self.get_map_name(self.roads)
        self.waypoints = generate_waypoints(self.roads, 0.2) #@TODO: This functions should be implemented here instead in map_utils

    def _get_map(self,map_name):
        """
    ...
    def _get_param(self, param, line_str):
        ...
        Two parameters are given, the 'param' to parse, and the line number to parse 'line_str'
        """
        value = ""
    ...
    def _get_junction_ids(self, junctions):
        ...
        This is usefull for getting an specific junction in junctions using its id
        """
        junction_ids = []
    ...
    def get_map_name(self, roads):
        ...
        Args:
            roads: roads already parsed from the xodr map

        Returns:
            map_name: (str) name of the map
        """

        if int(roads[0].length) == 36:
            return("Town01")

        elif int(roads[0].length) == 95:
            return("Town02")

        elif int(roads[0].length) == 43:
            return("Town03")

        elif int(roads[0].length) == 34:
            return("Town04")

        elif int(roads[0].length) == 53:
            return("Town05")

        elif int(roads[0].length) == 3:
            return("Town06")

        elif int(roads[0].length) == 33:
            return("Town07")

        elif int(roads[0].length) == 67:
            return("Town10")

        elif int(roads[0].length) == 13:
            return("Town10HD")
    ...
    def get_waypoint(self, location):
        ...
        Args:
            location: (T4ac_Location) xyz of the point

        Returns:
            closer_waypoint: (T4ac_Waypoint)
        """

        closer_distance = 10000 # High arbitrary value
        for wp in self.waypoints:
            distance = math.sqrt((wp.transform.location.x-location.x)**2 + 
                                 (wp.transform.location.y-location.y)**2 +
                                 (wp.transform.location.z-location.z)**2)
...
def main():
    ...
    map_object = MapParser("Town10HD")
    #print(len(map_object.roads))
    
    for road in map_object.roads:
        if (road.id == "18"):
            print(road.length)
            for _object in road.objects:
                if(_object.outline):
                    print(_object.id)
                    for corner in _object.outline:
                        print(corner.u, corner.v, corner.z)
